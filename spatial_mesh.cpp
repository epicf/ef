...
void Spatial_mesh::allocate_ongrid_values( )
{
    ...
    electric_field = (Vec3d ***) malloc( nx * sizeof(Vec3d **) );
    if ( ( charge_density == NULL ) || 
	 ( potential == NULL ) || 
	 ( electric_field == NULL ) ) {
	std::cout << "allocate_arrays_for_ongrid_values: x: out of memory " 
		  << std::endl;
	exit( EXIT_FAILURE );	
    }
    for( int i = 0; i < nx; i++) {
	charge_density[i] = (double **) malloc( ny * sizeof(double *) );
	potential[i] = (double **) malloc( ny * sizeof(double *) );
	electric_field[i] = (Vec3d **) malloc( ny * sizeof(Vec3d *) );
	if ( ( charge_density[i] == NULL ) || 
	     ( potential[i] == NULL ) || 
	     ( electric_field[i] == NULL ) ) {
	    std::cout << "allocate_arrays_for_ongrid_values: y: out of memory " 
		      << std::endl;
	    exit( EXIT_FAILURE );	
	}
    }   
    for( int i = 0; i < nx; i++) {
	for( int j = 0; j < ny; j++) {
	    charge_density[i][j] = (double *) calloc( nz, sizeof(double) );
	    potential[i][j] = (double *) calloc( nz, sizeof(double) );
	    electric_field[i][j] = (Vec3d *) calloc( nz, sizeof(Vec3d) );
	    if ( ( charge_density[i][j] == NULL ) || 
		 ( potential[i][j] == NULL ) || 
		 ( electric_field[i][j] == NULL ) ) {
		std::cout << "allocate_arrays_for_ongrid_values: z: out of memory " 
			  << std::endl;
		exit( EXIT_FAILURE );	
	    }
	}
...
Spatial_mesh::~Spatial_mesh()
{
    ...
    for( int i = 0; i < nx; i++) {
	for( int j = 0; j < ny; j++) {
	    free( charge_density[i][j] );
	    free( potential[i][j] );
	    free( electric_field[i][j] );
	}
    }
    for( int i = 0; i < nx; i++) {
