...
def get_B_field( h5file ):
    B_field = h5file["/ExternalMagneticField"].attrs["external_magnetic_field_z"][0]
...
def get_zlim( h5file ):
    start_z = h5["/ParticleSources/cathode_emitter"].attrs["box_z_near"][0]
...
plt.plot(h5["/ParticleSources/cathode_emitter/position_z"][:]*SI_conv_cm_to_m, 
            (h5["/ParticleSources/cathode_emitter/position_x"][:]*SI_conv_cm_to_m - center_of_beam),
