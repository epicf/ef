...
class Field_solver {
  ...
  private:
    ...
    // Solve potential
    void solve_poisson_eqn_iterative( Spatial_mesh &spat_mesh,
                                      Inner_regions_manager &inner_regions );
    void init_current_phi_from_spat_mesh_phi( Spatial_mesh &spat_mesh );
    void single_solver_iteration( Spatial_mesh &spat_mesh,
                                  Inner_regions_manager &inner_regions );
    void set_phi_next_at_boundaries();
    void compute_phi_next_at_inner_points_Jacobi( Spatial_mesh &spat_mesh );
    void compute_phi_next_at_inner_points_SOR( Spatial_mesh &spat_mesh );
    void set_phi_next_at_inner_regions( Inner_regions_manager &inner_regions );
    bool iterative_solution_converged( Spatial_mesh &spat_mesh );
